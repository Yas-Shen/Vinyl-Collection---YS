...
function displayVinyl(array $vinylCollection): string
{
    ...
    foreach ($vinylCollection as $vinyl) {
        if (array_key_exists('image', $vinyl) && array_key_exists('artist', $vinyl)
            && array_key_exists('album', $vinyl)
        && array_key_exists('year', $vinyl)
        && array_key_exists('genre', $vinyl)
        && array_key_exists('label-released', $vinyl)) {
